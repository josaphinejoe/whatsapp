...
class MockContactService implements ContactService {
  ...
  Future<List<Contact>> getContactList() async {
    if (this._contactList.isEmpty) await this._initialiseContactList();
  ...
  Future<void> sendMessage(String receiverPhone, String msg, bool isImage) async {
    ...
    contact.chats
        .add(MessageInfo(message: msg, time: DateTime.now().millisecondsSinceEpoch, isMyMsg: true, isImage: isImage));
    this._eventAggregator.publish(MessageSentEvent(contact));

    await Future.delayed(const Duration(seconds: 10), () {
      contact.chats
          .add(MessageInfo(message: "Hi, How are you?", time: DateTime.now().millisecondsSinceEpoch, isMyMsg: false));
      this._eventAggregator.publish(MessageSentEvent(contact));
    });
  ...
  void _mockContactList() {
    Contact hebbaContact = Contact(
        firstName: 'Hebba',
        lastName: 'Jose',
        phone: '9947367259',
        profilePicture: "https://miro.medium.com/v2/resize:fit:800/1*avN_YJhMr_MJZ48a-RbZzw.jpeg");

    hebbaContact.chats.add(MessageInfo(message: 'send me 2k', time: 1702009124942, isMyMsg: false));

    hebbaContact.chats.add(MessageInfo(message: 'ok', time: 1702009124942, isMyMsg: true));

    _contactList.add(hebbaContact);

    Contact johnlyContact = Contact(
        firstName: 'Johnly',
        phone: '9123456789',
        profilePicture:
    ...
    Contact meghanaContact = Contact(
        firstName: 'Meghana',
        lastName: 'Dhaware',
        phone: '9876543210',
        profilePicture:
    ...
    meghanaContact.chats.add(MessageInfo(message: 'Hi Meghana, Happy Birthday', time: 1701936000000, isMyMsg: true));

    meghanaContact.chats.add(MessageInfo(message: 'Thank you very much Irene', time: 1701940815000, isMyMsg: false));

    _contactList.add(meghanaContact);

    Contact roseContact = Contact(
        firstName: 'Rose',
        lastName: 'Mary',
        phone: '98765432345',
        profilePicture:
    ...
    Contact sreeLakshmiContact = Contact(
        firstName: 'Sree',
        lastName: 'Lakshmi',
        phone: '9476543210',
        profilePicture: "https://mastimorning.com/wp-content/uploads/2023/07/Best-HD-girls-whatsapp-dp-Wallpaper.jpg");

    sreeLakshmiContact.chats.add(MessageInfo(
        message: 'I will not be there in eveng, have some urgent work.', time: 1701936000000, isMyMsg: true));

    sreeLakshmiContact.chats
        .add(MessageInfo(message: 'ok. let me know, if you are coming', time: 1701936000000, isMyMsg: false));
